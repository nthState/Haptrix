...
for index, js in enumerate(json_files):
    ...
    with open(os.path.join(jsonDirectory, js)) as json_file:
        ...
        for section in sections:
            if 'link' in section and section['link'] != "":
                ...
                print("With: {}".format(objectDic[section['link']]['urls']['appleHelpBook']))
                section['link'] = objectDic[section['link']]['urls']['appleHelpBook']
                
        # Replace image paths in sections
        for section in sections:
            if 'image' in section and section['image'] != "":
                print("Replacing: {}".format(section['image']))
                section['image'] = section['image'].replace("../source/", "")
                
        # Replace body image
        if 'bodyImage' in documentation:
            documentation['bodyImage'] = documentation['bodyImage'].replace("../source/", "")
    ...
    newFileName = documentation['urls']['appleHelpBook']
